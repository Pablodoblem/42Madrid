...
BASE_URL = 'https://www.teatroreal.es/es/temporada-actual/flamenco-real#toContent'
...
def extract_opera_info(url):
    try:
        ...
        print(f"Número de bloques de conciertos de flamenco encontrados: {len(opera_blocks)}")
...
def main():
    print("Inicio de la extracción de datos de conciertos de flamenco...")
    ...
    existing_df = read_existing_data(EXCEL_FILE, 'Flamenco')
    ...
    if not existing_df.empty:
        ...
        if not changes_made:
            write_log("No se encontraron cambios en los datos de los conciertos de flamenco.")
    else:
        write_log("Archivo Excel o pestaña 'Flamenco' no encontrados. Creando nuevo archivo/pestaña.")

    # Guardar los datos actualizados en el archivo Excel
    with pd.ExcelWriter(EXCEL_FILE, mode='a', if_sheet_exists='replace') as writer:
        current_df.to_excel(writer, sheet_name='Flamenco', index=False)
