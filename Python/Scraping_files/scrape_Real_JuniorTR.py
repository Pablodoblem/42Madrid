...
BASE_URL = 'https://www.teatroreal.es/es/temporada-actual/real-junior#toContent'
...
def extract_opera_info(url):
    try:
        ...
        print(f"Número de bloques de conciertos de El Real Junior encontrados: {len(opera_blocks)}")
...
def main():
    print("Inicio de la extracción de datos de conciertos de El Real Junior...")
    ...
    existing_df = read_existing_data(EXCEL_FILE, 'El_Real_Junior')
    ...
    if not existing_df.empty:
        ...
        if not changes_made:
            write_log("No se encontraron cambios en los datos de los conciertos de El Real Junior.")
    else:
        write_log("Archivo Excel o pestaña 'El Real Junior' no encontrados. Creando nuevo archivo/pestaña.")

    # Guardar los datos actualizados en el archivo Excel
    with pd.ExcelWriter(EXCEL_FILE, mode='a', if_sheet_exists='replace') as writer:
        current_df.to_excel(writer, sheet_name='El_Real_Junior', index=False)
