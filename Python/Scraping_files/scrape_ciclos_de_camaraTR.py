...
BASE_URL = 'https://www.teatroreal.es/es/temporada-actual/domingos-camara#toContent'
...
def extract_opera_info(url):
    try:
        ...
        print(f"Número de bloques de conciertos de música de cámara encontrados: {len(opera_blocks)}")
...
def main():
    print("Inicio de la extracción de datos de conciertos de música de cámara...")
    ...
    existing_df = read_existing_data(EXCEL_FILE, 'Música_de_Cámara')
    ...
    if not existing_df.empty:
        ...
        if not changes_made:
            write_log("No se encontraron cambios en los datos de los conciertos de música de cámara.")
    else:
        write_log("Archivo Excel o pestaña 'Música_de_Cámara' no encontrados. Creando nuevo archivo/pestaña.")

    # Guardar los datos actualizados en el archivo Excel
    with pd.ExcelWriter(EXCEL_FILE, mode='a', if_sheet_exists='replace') as writer:
        current_df.to_excel(writer, sheet_name='Música_de_Cámara', index=False)
